...
class ReducerTest extends TestCase
{
    ...
    private function createShuffledFile(string $filename, array $groups): void
    {
        ...
        if ($handle !== false) {
            foreach ($groups as $key => $values) {
                // Write each value as a separate line (NEW format)
    ...
    public function testFloatKeys(): void
    {
        ...
        // Manually write file with float keys (NEW format: individual key-value pairs)
        $handle = fopen($file, 'w');
        if ($handle !== false) {
            fwrite($handle, serialize([
                'key' => 1.5,
    ...
    public function testComplexKeys(): void
    {
        ...
        // Manually write file since PHP arrays can't have array keys (NEW format: individual key-value pairs)
        $handle = fopen($file, 'w');
        if ($handle !== false) {
            fwrite($handle, serialize([
                'key' => $complexKey1,
    ...
    public function testBooleanKeys(): void
    {
        ...
        // Manually write file with boolean keys (NEW format: individual key-value pairs)
        $handle = fopen($file, 'w');
        if ($handle !== false) {
            fwrite($handle, serialize([
                'key' => true,
